                            ...
                            .addOnSuccessListener { querySnapshot ->
                                ...
                                if (!querySnapshot.isEmpty) {
                                    ...
                                    prefs.edit().apply {
                                        ...
                                        putString("staff_name", doc.getString("name"))
                                        putString("staff_phone", doc.getString("phone"))
                                        putString("staff_email", doc.getString("email"))
                                        putString("staff_department", doc.getString("departmentName"))
                                        putString("staff_college", doc.getString("collegeName"))
                                        putString("staff_course", doc.getString("course"))
                                        val years = doc.get("years") as? List<String> ?: emptyList()
                                        putStringSet("staff_years", years.toSet())
                                        putBoolean("is_logged_in", true)
                                        apply()
                                    }
                                    android.widget.Toast.makeText(context, "Login Successful", android.widget.Toast.LENGTH_SHORT).show()
                                    navController.navigate("staff_announcements") {
                                        popUpTo("landing")
                                    }
                                } else {
